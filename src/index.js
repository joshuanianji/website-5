...
import { projIcons, defaultIcon } from './assets/js/projectIcons';

// Flare stuff
import { FlareExample } from './assets/js/FlareExample';
...
window.onload = function () {
    ...
    console.log('./src/hex.flr'.constuctor)
    req.open("GET", './src/hex.flr', true);
    ...
    // flare
    let hex_animation;
    hex_animation = new FlareExample(
        document.getElementById('hex'),
        function () {
            hex_animation.load('./hex.flr', function (error) {
                if (error) {
                    console.log('failed to load actor file...', error);
                } else {
                    console.log('Successfully loaded flare object');
                }
            });
        }
    );
